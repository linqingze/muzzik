...
// ViewPagerController
@interface ScrollVCBase () <UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate>
...
- (IBAction)handleTapGesture:(id)sender {
    ...
    __weak ScrollVCBase *weakSelf = self;
...
- (MyTabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[_tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        MyTabView *tabView = [[MyTabView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.tabWidth, self.tabHeight)];
        [tabView addSubview:tabViewContent];
        [tabView setClipsToBounds:YES];
        [tabView setIndicatorColor:self.indicatorColor];
        
        tabViewContent.center = CGPointMake(tabView.center.x, tabView.center.y-4);
