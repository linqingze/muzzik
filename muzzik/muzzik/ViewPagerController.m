...
#define navHeight 0.0;      //导航栏高度
#define tabBarHeight 56.0;
#define kDefaultStartFromSecondTab 1.0 // 1.0: YES, 0.0: NO

#define kDefaultCenterCurrentTab 1.0 // 1.0: YES, 0.0: NO
...
- (void)viewWillLayoutSubviews {
    ...
    frame.origin.x = 0.0;
    frame.origin.y = self.tabHeight+navHeight;
    frame.size.width = self.view.bounds.size.width;
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    UIView *fview =   activeTabView.subviews[0];
    
    UIImageView *LABEL =  fview.subviews[0];
    if (LABEL.tag ==10000) {
        [LABEL setImage:[UIImage imageNamed:@"homeImage"]];
    }
    else if (LABEL.tag ==10001) {
        [LABEL setImage:[UIImage imageNamed:@"recommendImage"]];
    }
    else if (LABEL.tag ==10002) {
       [LABEL setImage:[UIImage imageNamed:@"profileImage"]];
    }
    else if (LABEL.tag ==10003) {
        [LABEL setImage:[UIImage imageNamed:@"notiImage"]];
    }
    else{
        [LABEL setImage:[UIImage imageNamed:@"searchImage"]];
    }
    
    
//    UIImageView *img = fview.subviews[1];
//    [UIView animateWithDuration:0.3 animations:^{
//        [img setFrame:CGRectMake(SCREEN_WIDTH/6.0, 5, 0, 0)];
//    }];
    ...
    activeTabView.selected = YES;
     UIView *nview =   activeTabView.subviews[0];
    LABEL =  nview.subviews[0];
    
    if (LABEL.tag == 10000) {
        [LABEL setImage:[UIImage imageNamed:@"homeImage_click"]];
    }
    else if (LABEL.tag == 10001) {
        [LABEL setImage:[UIImage imageNamed:@"recommendImage_click"]];
    }
    else if (LABEL.tag == 10002) {
        [LABEL setImage:[UIImage imageNamed:@"profileImage_click"]];
    }
    else if (LABEL.tag == 10003) {
        [LABEL setImage:[UIImage imageNamed:@"notiImage_click"]];
    }
    else{
        [LABEL setImage:[UIImage imageNamed:@"searchImage_click"]];
    }
    //img = nview.subviews[1];
//    [UIView animateWithDuration:0.3 animations:^{
//        [img setFrame:CGRectMake(SCREEN_WIDTH/6.0-5, 0, 10, 10)];
//    }];
